...
//
//	  2000-MAR-07 - Thanks to Ullrich Pollahne for catching a range bug in one
...
//	#pragma setlocale ("kor")
	...
	#pragma warning (disable : 4311) // 'reinterpret_cast' : 'x'에서 'y'(으)로 포인터가 잘립니다.
	...
	#pragma warning (disable : 4996) // This function or variable may be unsafe.
		...
		#define PETER_UNDEF_SCL_SECURE_NO_WARNINGS
		#define _SCL_SECURE_NO_WARNINGS
	#endif
	#ifndef _CRT_SECURE_NO_WARNINGS
		#define PETER_UNDEF_CRT_SECURE_NO_WARNINGS
...
inline void	ssasn(std::string& sDst, const std::wstring& sSrc)
{
	...
	else
	{
		...
		// 20071123 peter: 이전까지 문제가 없었으나 unicode build에서 ansi로 변경하는 경우 문제 발생
		// 위에는 3바이트 문자가 어쩌고 하는데, 나중에 문제가 되면 *3으로 변경하면 된다.
//#ifdef SS_MBCS
		nDst *= 2;
//#endif
...
inline void	ssadd(std::string& sDst, const std::wstring& sSrc)
{
	...
	if ( nSrc > 0 )
	{
		...
		// 20071123 peter: 마찬가지로 문제 발생하게 되어 있다.
		nAdd *= 2;
//#ifdef SS_MBCS
//		nAdd		= static_cast<int>(static_cast<double>(nAdd) * 2);
//#endif
...
public:
	...
	// They also are compiled if you have explicitly turned off refcounting
		...
		MYTYPE& assign(const MYTYPE& str, MYSIZE nStart, MYSIZE nChars)
		{
			...
			// value.  Thanks to Ullrich Pollahne for catching this bug
...
// Turn back on any Microsoft warnings we turned off.
#if defined(_MSC_VER) && (_MSC_VER > 1100)
	#pragma warning(pop)
#endif
...
#ifdef PETER_UNDEF_SCL_SECURE_NO_WARNINGS
	#undef _SCL_SECURE_NO_WARNINGS
#endif

#ifdef PETER_UNDEF_CRT_SECURE_NO_WARNINGS
